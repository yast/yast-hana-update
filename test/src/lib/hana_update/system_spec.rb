  ...
  describe '#resource_force' do
    context 'on remote node' do
      it 'forces resource start' do
        ...
        _, status = HANAUpdater::System.resource_force(const.resources[:vip], :start, node: const.remote.host_name)
        expect(status.exitstatus).to eq 0
      end
      ...
      it 'forces resource stop' do
        ...
        _, status = HANAUpdater::System.resource_force(const.resources[:vip], :stop, node: const.remote.host_name)
        expect(status.exitstatus).to eq 0
      end
      ...
      it 'forces resource check' do
        ...
        _, status = HANAUpdater::System.resource_force(const.resources[:vip], :check, node: const.remote.host_name)
        expect(status.exitstatus).to eq 0
      end

      it 'does not accept other verbs' do
        expect {HANAUpdater::System.resource_force(const.resources[:vip], :move, node: const.remote.host_name)}.to raise_error(ArgumentError)
      end
    end
    ...
    context 'on local node' do
      ...
      it 'does not accept other verbs' do
        expect {HANAUpdater::System.resource_force(const.resources[:vip], :move)}.to raise_error(ArgumentError)
      end
    end
  end
  ...
  describe '#recursive_copy' do
    context 'on local node' do
      it 'copies the update medium' do
        ...
        expect(Dir).to receive(:exists?).with(destination_path).and_return(true)
        expect_syscall(
            type: :output,
